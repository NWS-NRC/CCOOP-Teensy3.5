...
void unitInfo()
{
  ...
  usb.println(F("Software Version: v0.8"));
...
/*void process_FileTransferRequest()
  ...
  if (xbee.available() > 0)
  {
    while (xbee.available() > 0 || info_transfer == false)
    {
      ...
      if (d == '\r' || d == '\n')
      {
        ...
        while (xbee.available() > 0)
        {
          byte c  = xbee.read(); //empty out remaining serial info from buffer
        }
      }
      else
      {
  ...
  byte chunks = f_size / 50;
  byte last_chunk = f_size % 50;
  ...
  if (myFile)
  {
    ...
    for (byte i = 0; i < chunks; i++)
    {
      ...
      if (bad_transfers < 5)
      {
        ...
        while (xbee.available() < 50 && (millis() - file_timer < 10000UL))
        {
          delay(10);
        }
        if (xbee.available() == 50)
        {
          byte chunk_data[50];
          byte cnt = 0;
          while (xbee.available() > 0)
          ...
          if (cnt == 50)
          {
            chksum = (chksum % 256);
            usb.println();
            usb.println(F("50 Bytes Received"));
            myFile.write(chunk_data, 50);
            good_rx = true;
          }
          else
          {
            usb.println(F("Invalid transfer read"));
            good_rx = false;
            bad_transfers += 1;
          }
        }
    ...
    while (file_complete == false && bad_transfers < 5)
    {
      ...
      if (xbee.available() == last_chunk)
      {
        ...
        if (cnt == last_chunk)
        {
          chksum = (chksum % 256);
          usb.println();
          usb.println(F("Final Transfer Received"));
          good_rx = true;
