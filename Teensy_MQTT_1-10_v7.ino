...
void loop()
{
  /*************************Checking if ESP32 needs reset*************************/
  if (millis() < espResetTimer) //checking for millis() rollover
  {
    espResetTimer = millis(); //reset timer if so
  }
  if ((millis() - espResetTimer) > 600000UL) //if timer runs over 10 minutes with no comms from ESP reset it.
  {
    resetESP();
  }
  ...
  if (dActive == true && minute % 15 == 0 && dInfo_Sent == false)
  {
    checkFPRD();
    lastPrecipTx = minute;
  }

  if (dActive == true && minute % 15 != 0 && dInfo_Sent == true)
  {
    dInfo_Sent = false;
  }
...
void getCurrentTemp()
{
  ...
  if (boot == 0)
  {
    ...
    if (match == true)
    {
      ...
      {
        if (((lastTemp - 300) <= tempCheck) && ((lastTemp + 300) >= tempCheck)) // +/- 30 degree check
        {
...
void unitInfo()
{
  ...
  usb.println(F("CCOOP"));
  usb.println(F("Software Version: v0.3"));
  usb.println(F("Dated: 5/26/19\n"));
}
...
void checkFPRD()
{
  ...
  if (fpr.available() > 0)
  {
    ...
    {
      char inchar = char(fpr.read()); //clear out any data in the MMTS comms buffer
      usb.print(inchar);
      delay(1000);
      if (inchar == '>')
      {
