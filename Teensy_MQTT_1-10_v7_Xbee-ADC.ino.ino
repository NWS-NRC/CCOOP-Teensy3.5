...
void getCurrentTemp()
{
  ...
  if (boot == 0)
  {
    usb.println(F("Checking Temp"));
    getThermReading();
    if (dActive == false) {
      checkFPR();
    }
    if ((outThermTemp > -80.0) && (outThermTemp < 200.0))
    {
      if (((lastTemp - 100) <= int(outThermTemp * 10)) && ((lastTemp + 100) >= int(outThermTemp * 10))) // +/- 10 degree check
      {
        temperature = int(outThermTemp * 10);
  ...
  {
    usb.println(F("Checking Temp, boot1"));
    getThermReading();
    if (dActive == false) {
      checkFPR();
    }
  ...
  usb.println(float(temperature) / 10);
  if (boot == 0)
  {
